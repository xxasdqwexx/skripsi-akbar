	...
	<section class="cart bgwhite">
		<div class="container">
			...
			<div align="center" class="mb-3">
  				<button name="filter" type="submit" class="btn btn-success btn-sm">submit</button>
  				<a type="button" class="btn btn-primary btn-sm" href="<?php echo base_url()."Transaksi/lihat";?>">reset</a>
  			  <button class="btn btn-info btn-sm"  onclick="window.print()">print</button>
			<div align="center" class="mb-3">
  			</div>
			</form>

			<?php foreach ($transaksi_satu as $transaksi1) : ?>
				<?php
				if (isset($_POST['filter'])) {
					echo $dari_tgl. " s/d "  .$sampai_tgl;
				}
				?>
				<br>
				<span id="val" style="text-align: right;">Total Penjualan : Rp. <?php echo number_format($transaksi1->subtotal,'0',',','.') ?></span>
				<?php endforeach; ?>
			<br>
			</div>			
				<div class="wrap-table-shopping-cart bgwhite">
					<table class="table-shopping-cart">
						<tr align="left" class="table-head">
							<th class="column-1">NO</th>
							<th class="column-2">KODE</th>
							<th class="column-2">NAMA</th>
							<th class="column-2">TANGGAL</th>
							<th class="column-2">PEMBAYARAN</th>
							<th class="column-1">TOTAL</th>
							<th class="column-4">KEMBALIAN</th>
						...
						<td class="column-2"><a href="<?php echo base_url()."Transaksi/detail/".$transaksi->id_transaksi;?>"><u><b><?= $transaksi->id_transaksi ?></b></u></a></td>
						<td class="column-2"><?= $transaksi->nama ?></td>
						<td class="column-2"><?= date('d-m-Y H:i:s', $transaksi->id_transaksi ) ?></td>
							<?php if ($this->session->userdata('akses_level') == 'admin') {
								?>
						<td class="column-2">Rp. <?php echo number_format($transaksi->tunai,'0',',','.')?></b></u></td>
							<?php }else {?>
							<?php }?>
							<?php if ($this->session->userdata('akses_level') == 'admin') {
								?>
						<td class="column-1"><input value="<?php if ($transaksi->tunai!==null){echo number_format($transaksi->total,'0',',','.');} ?>" type="text" id="num1"/></td>
								<?php }else {?>
								<?php }?> 
						<td class="column-4">Rp. <?php if ($transaksi->tunai!==null) {
